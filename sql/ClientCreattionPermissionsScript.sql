# Client creation and permission assignment script for CNT 4714 Project 4 - Fall
2024
#
# IMPORTANT NOTE: Run all database creation scripts before running this script.
#
# Delete the client1, client2, users created for Project 3
# NOTE: If any of these users are currently connected to the MySQL server,
terminate their connections prior to running this script.
# If any of these clients are currently connected, the drop user command will not
take effect until the that user's connection is terminated.
...
create user if not exists "systemapp" identified with sha256_password by
"systemapp";
create user if not exists "theaccountant" identified with sha256_password by
"theaccountant";
